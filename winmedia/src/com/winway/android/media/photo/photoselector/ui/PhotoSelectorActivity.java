...
import com.winway.android.media.photo.photoselector.domain.PhotoSelectorDomain;
...
import com.winway.android.media.photo.photoselector.util.CommonUtils;
...
public class PhotoSelectorActivity extends Activity
	...
	public static int MAX_IMAGE;// 最大图片上限
	...
	public static ArrayList<PhotoModel> selected;// 选择的照片
	...
	public boolean onCheckedChanged(PhotoModel photoModel, CompoundButton buttonView, boolean isChecked) {
		// 判断是否已经达到图片选择上限数
		if (isChecked&&selected.size() >= MAX_IMAGE) {//点击勾选时才判断
			Toast.makeText(this, "最多只能选择 " + PhotoSelectorActivity.MAX_IMAGE + "张", Toast.LENGTH_SHORT).show();
		...
		if (selected.isEmpty()) {
			tvPreview.setEnabled(false);
			tvPreview.setText(getString(R.string.preview2));
		}
		return true;
	}
