package com.winway.android.media.photo.photoselector.controller;
...
public class AlbumController {
	...
	public List<AlbumModel> getAlbums() {
		...
		Cursor cursor = resolver.query(Media.EXTERNAL_CONTENT_URI,
				new String[] { ImageColumns.DATA, ImageColumns.BUCKET_DISPLAY_NAME, ImageColumns.SIZE }, null, null,
				null);
		if (cursor == null || !cursor.moveToNext())
			return new ArrayList<AlbumModel>();
		cursor.moveToLast();
		AlbumModel current = new AlbumModel("??????", 0, cursor.getString(cursor.getColumnIndex(ImageColumns.DATA)),
				true); // "??????"???
		albums.add(current);
		do {
	...
	public List<PhotoModel> getAlbum(String name) {
		Cursor cursor = resolver.query(
				Media.EXTERNAL_CONTENT_URI, new String[] { ImageColumns.BUCKET_DISPLAY_NAME, ImageColumns.DATA,
