package com.winway.android.ewidgets.textview;
...
public class ExpandableTextView extends TextView {
	...
	// used to judge if the listener of corresponding to the onclick event of
	// ExpandableTextView
	 ...
	 *            the width of ExpandableTextView in px unit, used to get max
	 *            line number of original text by given the width
	 * @param expandState
	 *            expand or shrink
	 */
	...
	private CharSequence getNewTextByConfig() {
		...
		switch (mCurrState) {
		case STATE_SHRINK: {
			mLayout = new DynamicLayout(mOrigText, mTextPaint, mLayoutWidth, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f,
					false);
			...
			int remainWidth = getValidLayout().getWidth()
					- (int) (mTextPaint.measureText(mOrigText.subSequence(indexStart, indexEndTrimmed).toString())
							+ 0.5);
			float widthTailReplaced = mTextPaint.measureText(getContentOfString(mEllipsisHint) + (mShowToExpandHint
		...
		case STATE_EXPAND: {
			...
			ssbExpand.setSpan(mTouchableSpan, ssbExpand.length() - getLengthOfString(mToShrinkHint), ssbExpand.length(),
	...
	/**
	 * Copy from: http://stackoverflow.com/questions
	 * /20856105/change-the-text-color-of-a-single-clickablespan-when-pressed-
	 * without-affecting-o By: Steven Meliopoulos
	 */
